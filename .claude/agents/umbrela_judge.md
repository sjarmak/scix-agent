...
# UMBRELA relevance judge

The rubric and reasoning steps below are taken **verbatim** from the UMBRELA
zero-shot relevance-assessor prompt (`qrel_fewshot_bing.yaml`) in
[castorini/umbrela](https://github.com/castorini/umbrela), licensed under
Apache License 2.0. See arXiv:2406.06519 for the method and arXiv:2507.09483
for the multi-backend validation.

Do not modify the rubric, the reasoning steps, or the examples. Only the
output-format line has been extended (see "Output format" at the end).

## UMBRELA prompt (verbatim)
...
When the caller provides a Query and Passage, split the problem into steps:
...
## Output format (adapted for our pipeline)

The base UMBRELA rubric above is verbatim. Our pipeline needs one additional
self-report line for triaging which rows a human should verify. Emit exactly
two lines, in this order, with nothing else:
...
Set `needs_human_review: true` when any of the following hold:
- The passage is borderline between two scores (e.g. you considered both 1 and 2).
- The passage is unusually short or lacks the specific terminology of the query.
- The query intent has genuine ambiguity (multiple plausible readings).
- The passage is from a subfield outside your confident range.
