...
## Amendment: 2026-04-18 — M6/M12 collapse to JIT-only

**Status:** Effective 2026-04-18. Triggered by the §Convergence Refinements → Preserved dissent clause.
**Amendment doc:** `docs/prd/amendments/2026-04-18-m6-m12-collapse.md`
**Hinge eval bead:** `scix_experiments-xz4.1.10` (M4 three-way eval on real data).
**Amendment bead:** `scix_experiments-xz4.1.21`.

M4 three-way eval on the populated 32.4M-paper corpus (50 citation-seeded queries + 20 agent graph-walk tasks) showed that the static-core retrieval lane regressed nDCG@10 by 14%, Recall@20 by 21%, and MRR by 17% versus hybrid baseline. The JIT re-rank lane was indistinguishable from baseline. Per the preserved-dissent clause, Advocate B wins:

- ~~**M6** as default retrieval pre-filter~~ — **struck from the rollout.** The curated-core materialized view remains for graph-analytics and diagnostics; it is not promoted onto the default retrieval path.
- ~~**M12** default static-core retrieval promotion~~ — **struck from the rollout.** The dual-lane architecture stays; its default-on retrieval behavior is removed.

**Scope of this amendment** is narrow: it kills (a) static-core hard-filter as a default retrieval pre-filter and (b) naive entity-overlap rerank as a default retrieval boost. It does **not** claim entity enrichment as a whole fails retrieval. The M4 eval tested two strawman lanes; the richer value props (alias expansion, ontology-metadata expansion, disambiguation, type filters, mention-specific filters, community expansion) were never exercised and are being re-evaluated in `scix_experiments-xz4.1.23` on a calibrated persona-Claude judge. Value-prop implementations live in `scix_experiments-xz4.1.24` (alias expansion), `xz4.1.25` (ontology-metadata parser), `xz4.1.26` (query-time disambiguator), and `xz4.1.27` (entity_type/entity_id filters).

See the amendment doc for full eval numbers, scope framing, follow-on work pointers, and paper-writeup guidance.
