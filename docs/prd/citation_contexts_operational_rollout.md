...
4. **A NAS pg_dump destination exists.** `/mnt/postgres/scix_dumps/`
   created, mode 0750, with at least the projected 79n.1 dump size
...
CREATE TABLE citation_contexts (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY,  -- BIGINT, see §id-overflow
...
new partitioned table uses `BIGINT GENERATED BY DEFAULT AS IDENTITY`
...
pgbench -d scix_test \
  ...
  -P 30 \
  --report-per-command \
  > /tmp/scix_bench_post_partition.log
```
...
pgbench -d scix \
  ...
  -P 30 \
  --report-per-command \
  > /tmp/scix_bench_pre_partition.log
```
...
sudo chmod 0750 /mnt/postgres/scix_dumps/
```
...
1. `pg_dump -d scix --schema=public --table='citation_contexts*' --table=v_claim_edges --format=custom --compress=6 --file /mnt/postgres/scix_dumps/citation_contexts_$(date +%Y%m%d).dump`
2. Verify dump with `pg_restore --list` (catalog read-only check).
3. Retain last 4 weekly dumps; rotate older to `/mnt/postgres/scix_dumps/archive/`.
4. Wrap in `$SCIX_BATCH` per CLAUDE.md memory-isolation rule —
...
pgbench -d scix -f ~/scix_bench_qmix.sql -c 4 -j 4 -T 300 -P 30 \
  --report-per-command > /tmp/scix_bench_post_prod.log
```
...
```bash
LATEST=$(ls -1t /mnt/postgres/scix_dumps/citation_contexts_*.dump | head -1)
