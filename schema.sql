...
-- 039_papers_body_gin.sql
-- Migration 039: GIN expression index on papers.body for body-only BM25.
--
-- Replaces the original migration 039 (papers_ads_body table) which was
-- dropped to free 231 GB of duplicated body text. papers.body (migration
-- 010) already holds the same data; an expression index gives identical
-- query semantics without storing a second copy.
--
-- The partial-index predicate excludes bodies > 1 MB (PostgreSQL's tsvector
-- hard limit) so they don't fail the index build. They remain searchable
-- via the raw papers.body column if needed.

BEGIN;

CREATE INDEX IF NOT EXISTS ix_papers_body_tsv
    ON papers USING GIN (to_tsvector('english', body))
    WHERE body IS NOT NULL AND length(body) <= 1048575;

COMMENT ON INDEX ix_papers_body_tsv IS
    'GIN expression index on body tsvector. Used by hybrid_search() body '
    'BM25 signal. Partial index excludes bodies > 1 MB (tsvector limit).';
