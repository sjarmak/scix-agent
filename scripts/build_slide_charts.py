...
def chart_retrieval_eval():
    ...
    ax.set_title(
        "50-query retrieval evaluation (citation-based ground truth)",
        fontsize=14,
        fontweight="bold",
        pad=14,
        color="#111",
...
def chart_corpus_stats():
    ...
    for ax, (label, value, sub) in zip(axes, stats):
        ...
        ax.text(0.5, 0.38, label, ha="center", va="center", fontsize=14, color="#222")
        ax.text(0.5, 0.22, sub, ha="center", va="center", fontsize=11, color="#666", style="italic")
...
def chart_architecture():
    ...
    # Storage layer
    box(
        3.2, 4.2, 3.0, 0.9, "PostgreSQL 16 + pgvector", "HNSW, halfvec, BM25/tsvector", color=ACCENT
    )
    ...
    box(
        ...
        0.9,
        "MCP server  ·  13 tools",
        "search · read_paper · citation_graph · find_gaps · ...",
        color="#db8d3a",
    )
    # Agent
    ...
    ax.text(
        6,
        5.25,
        "Architecture: single-box Postgres powers an agent-native knowledge layer",
        ha="center",
        fontsize=14,
        fontweight="bold",
        color="#111",
...
def chart_tool_count():
    ...
    ax.set_title(
        "Tool count discipline: 22 → 13", fontsize=14, fontweight="bold", pad=14, color="#111"
    )
...
def chart_embedding_pipeline():
    ...
    ax.set_title(
        "32 → 508 rec/s: embedding pipeline optimization",
        fontsize=14,
        fontweight="bold",
        pad=14,
        color="#111",
