...
OUT_PATH = Path(
    "/home/ds/projects/scix_experiments/docs/slides/mlops_community_research_copilot.pptx"
)
...
def add_bullets(
    slide, bullets, left, top, width, height, *, size=18, color=INK, bullet_color=ACCENT
):
    tb = slide.shapes.add_textbox(left, top, width, height)
    tf = tb.text_frame
    tf.margin_left = tf.margin_right = Emu(0)
...
def add_footer(slide, idx, total):
    add_text(
        slide,
        "Stephanie Jarmak  ·  MLOps Community  ·  Making Scientific Knowledge Navigable for Agents",
        Inches(0.5),
        Inches(7.1),
        Inches(10),
        Inches(0.3),
...
def add_title(slide, text, *, sub=None, top=Inches(0.55)):
    add_text(
        slide, text, Inches(0.5), top, Inches(12.3), Inches(0.8), size=32, bold=True, color=INK
    )
...
add_text(
    s,
    "Sourcegraph  ·  Harvard & Smithsonian | Center for Astrophysics",
    Inches(0.9),
    Inches(4.9),
    Inches(12),
    Inches(0.45),
...
add_text(
    s,
    "MLOps Community · Research Copilot Workshop",
    Inches(0.9),
    Inches(6.6),
    Inches(12),
    Inches(0.4),
...
stat(Inches(0.5), "3M+", "scientific papers published per year", "and doubling every ~12 years")
stat(Inches(4.9), "20–30%", "of a researcher's time", "spent on literature search")
stat(Inches(9.3), "~10", "results per search query", "no graph, no memory, no structure")

add_text(
    s,
    "A research copilot built on vanilla search is stuck with the same primitives as the human — scaled up.",
    Inches(0.5),
    Inches(5.6),
    Inches(12.3),
    Inches(0.6),
...
add_title(
    s,
    'What "navigable" means for an agent',
...
def column(x, title, items, title_color):
    add_text(s, title, x, top, col_w, Inches(0.5), size=20, bold=True, color=title_color)
    add_rule(s, x, top + Inches(0.55), Inches(0.6), color=title_color)
    for i, (head, body) in enumerate(items):
        y = top + Inches(0.9 + i * 0.95)
        add_text(s, head, x, y, col_w, Inches(0.35), size=15, bold=True, color=INK)
        add_text(s, body, x, y + Inches(0.35), col_w, Inches(0.55), size=13, color=INK_MUTED)
...
for i, (num, head, body) in enumerate(sections):
    ...
    add_text(
        s, body, Inches(1.8), y + Inches(0.45), Inches(11), Inches(0.4), size=14, color=INK_MUTED
    )
...
add_title(
    s,
    "Search APIs return ranked lists. Agents need a knowledge structure.",
...
missing = [
    ...
    ("No session memory", "Every query starts cold. Working sets live in the prompt or nowhere."),
    (
...
for i, (h, b) in enumerate(missing):
    y = Inches(2.0 + i * 0.95)
    add_text(s, h, Inches(0.6), y, Inches(5), Inches(0.5), size=18, bold=True, color=ACCENT_DARK)
    add_text(s, b, Inches(5.8), y + Inches(0.05), Inches(7.0), Inches(0.9), size=14, color=INK)
...
s = add_blank(prs)
set_bg(s)
add_title(s, "Why MCP", sub="Model Context Protocol is a good fit for agent-native knowledge")

reasons = [
    (
        "Tool-calling primitives",
        "First-class: name, schema, typed args, typed returns. The agent knows " "what it can do.",
    ),
    (
        "Session context",
        "Stateful per-connection. Working sets live on the server, not in the prompt.",
    ),
    ("Structured responses", "JSON objects, not HTML snippets. Compose cleanly with other tools."),
    (
        "Transport agnostic",
...
for i, (h, b) in enumerate(reasons):
    y = Inches(2.0 + i * 0.95)
    add_text(s, h, Inches(0.6), y, Inches(4.3), Inches(0.5), size=18, bold=True, color=ACCENT_DARK)
    add_text(s, b, Inches(5.0), y + Inches(0.05), Inches(7.8), Inches(0.9), size=14, color=INK)
...
s = add_blank(prs)
set_bg(s)
add_title(s, "The corpus", sub="NASA ADS — the canonical bibliographic database for astronomy")
add_image(s, ASSETS / "corpus_stats.png", Inches(0.5), Inches(2.0), width=Inches(12.3))
# secondary stats
add_text(
    s,
    "14.9M papers with full body text (46%)  ·  PostgreSQL 16 + pgvector  ·  single server (62 GB RAM)",
    Inches(0.5),
    Inches(6.4),
    Inches(12.3),
    Inches(0.5),
...
add_title(
    s,
    "Corpus completeness determines graph validity",
...
add_text(
    s,
    "82.2% of edges dangle into the void when you index a rolling window. That makes PageRank, community detection, and co-citation analysis structurally wrong — not just incomplete.",
    Inches(0.8),
    Inches(6.5),
    Inches(11.7),
    Inches(0.8),
...
card(
    Inches(0.8), Inches(2.0), Inches(5.9), Inches(4.5), fill=RGBColor(0xFD, 0xEA, 0xE8), border=RED
)
add_text(
    s,
    "6-year window",
...
rows_left = [
    ("~45,000", "disconnected components"),
    ("~1.7M", "isolated nodes"),
...
for i, (big, small) in enumerate(rows_left):
    y = Inches(3.2 + i * 0.75)
    add_text(s, big, Inches(1.1), y, Inches(2.2), Inches(0.5), size=22, bold=True, color=INK)
    add_text(
        s, small, Inches(3.4), y + Inches(0.1), Inches(3.3), Inches(0.4), size=13, color=INK_MUTED
    )
...
rows_right = [
    ("19.98M", "in the giant component (99.3% of connected)"),
...
for i, (big, small) in enumerate(rows_right):
    y = Inches(3.2 + i * 0.75)
    add_text(s, big, Inches(7.3), y, Inches(2.5), Inches(0.5), size=22, bold=True, color=INK)
    add_text(
        s, small, Inches(9.9), y + Inches(0.1), Inches(2.9), Inches(0.4), size=13, color=INK_MUTED
    )

add_text(
    s,
    "Only 1 connected component exceeds 100 nodes — the giant component itself.",
    Inches(0.5),
    Inches(6.7),
    Inches(12.3),
    Inches(0.4),
...
add_title(
    s,
    "Takeaway: corpus completeness is a retrieval-quality axis",
...
for i, (h, b) in enumerate(callouts):
    ...
    card(
        left, Inches(2.2), Inches(4.0), Inches(4.3), fill=RGBColor(0xF5, 0xF8, 0xFD), border=ACCENT
    )
    add_text(
        s,
        h,
    ...
    add_text(s, b, left + Inches(0.3), Inches(3.4), Inches(3.5), Inches(2.7), size=14, color=INK)
...
add_text(
    s,
    "Deliberate simplicity: no Elasticsearch, no separate vector DB, no orchestrator. Postgres + pgvector does it all.",
    Inches(0.5),
    Inches(6.8),
    Inches(12.3),
    Inches(0.4),
...
add_title(
    s,
    "Retrieval stack: three signals, reciprocal rank fusion",
...
# RRF box
rrf = card(Inches(2.6), Inches(5.7), Inches(8.1), Inches(1.0), fill=ACCENT_SOFT, border=ACCENT_DARK)
add_text(
    s,
    "Reciprocal Rank Fusion (k=60)  →  hybrid_search",
    Inches(2.6),
    Inches(5.9),
    Inches(8.1),
    Inches(0.7),
...
add_title(
    s,
    "pgvector + Qdrant: complementary, not competing",
...
def _vcard(left, title, sub, lines, color):
    r = s.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, left, Inches(2.0), Inches(6.0), Inches(4.7))
...
add_title(
    s,
    "Prototype: find_similar_by_examples (14th MCP tool)",
    sub='"More like these papers, less like those" — optionally filtered',
)

# Left: signature + state
_sig_bg = s.shapes.add_shape(
    MSO_SHAPE.RECTANGLE, Inches(0.5), Inches(1.95), Inches(5.9), Inches(3.1)
)
_sig_bg.line.color.rgb = RULE
_sig_bg.line.width = Pt(1)
_sig_bg.fill.solid()
_sig_bg.fill.fore_color.rgb = RGBColor(0xF8, 0xFA, 0xFC)
add_text(
    s,
...
for i, (big, small) in enumerate(_state_rows):
    ...
    add_text(
        s, small, Inches(3.0), y, Inches(3.3), Inches(0.3), size=11, italic=True, color=INK_MUTED
    )

# Right: demo result excerpt
add_text(
    s,
    "Live demo — seed: two graphene/terahertz papers",
    Inches(6.7),
    Inches(2.0),
    Inches(6.3),
    Inches(0.4),
...
add_title(
    s,
    "Graph intelligence: topology, communities, multi-hop moves",
...
def graph_col(left, title, lines):
    add_text(s, title, left, col_y, col_w, Inches(0.5), size=18, bold=True, color=ACCENT_DARK)
...
set_bg(s)
add_title(
    s, "13 MCP tools, down from 22", sub="Premortem: >15 tools degrades agent selection accuracy"
)
add_image(s, ASSETS / "tool_count.png", Inches(2.0), Inches(2.0), width=Inches(9.3))
# right-side list of current tools
tools = (
    "search · concept_search · get_paper · read_paper · citation_graph · "
...
add_title(
    s,
    "Tool design for agents ≠ tool design for humans",
...
principles = [
    ...
    ("Stable, typed interfaces", "JSON schema. No free-text fields. Every response parses."),
    (
        "Include provenance",
        "Which tool returned this? Which query? Agent can reason about its own trail.",
    ),
    ("Idempotent reads", "Same inputs → same outputs. Caching is trivial."),
    ("Cursor-based pagination", "Big result sets return a cursor, not 10,000 bibcodes."),
]
for i, (h, b) in enumerate(principles):
    y = Inches(2.0 + i * 0.95)
    add_text(s, h, Inches(0.6), y, Inches(4.3), Inches(0.5), size=17, bold=True, color=ACCENT_DARK)
    add_text(s, b, Inches(5.0), y + Inches(0.05), Inches(7.8), Inches(0.9), size=14, color=INK)
...
add_title(
    s,
    "Session state: the agent's short-term memory",
    sub="In-memory working set per MCP connection — not in the prompt, not in a DB",
)

# ASCII-ish code block visual
code_bg = s.shapes.add_shape(
    MSO_SHAPE.RECTANGLE, Inches(0.6), Inches(2.0), Inches(6.4), Inches(4.6)
)
...
add_text(
    s, code, Inches(0.8), Inches(2.2), Inches(6.0), Inches(4.4), size=13, color=INK, font="Menlo"
)
...
for i, (h, b) in enumerate(unlocks):
    y = Inches(2.9 + i * 0.9)
    add_text(s, h, right_x, y, Inches(5.4), Inches(0.35), size=15, bold=True, color=INK)
    add_text(s, b, right_x, y + Inches(0.35), Inches(5.4), Inches(0.5), size=12, color=INK_MUTED)
...
add_title(
    s,
    "Three community lenses — and their informative disagreements",
    sub="Each lens has clean semantics. The agent picks.",
)


def lens(left, title, sub, items, color):
    card(left, Inches(2.0), Inches(4.0), Inches(4.5), fill=RGBColor(0xFA, 0xFB, 0xFD), border=color)
    add_text(
        s,
        title,
        left + Inches(0.3),
...
s = add_blank(prs)
set_bg(s)
add_title(s, "Live demo", sub="A research copilot maps the exoplanet-atmospheres landscape")

# Demo task card
dc = card(Inches(0.5), Inches(2.0), Inches(7.0), Inches(4.6), fill=ACCENT_SOFT, border=ACCENT)
add_text(
    s,
    "User prompt",
    Inches(0.8),
...
for i, (n, tool, body) in enumerate(steps):
    ...
    add_text(s, body, Inches(10.6), y, Inches(2.5), Inches(0.4), size=11, color=INK_MUTED)
...
add_title(
    s,
    "Agent trace: tool calls visualized over time",
...
add_title(
    s,
    "UMAP of 32M INDUS embeddings — the research landscape the agent navigates",
...
add_title(
    s,
    "Citation flow between communities",
...
add_text(
    s,
    "These are the structural views an agent reasons over — not visually, but as graph edges and community IDs.",
    Inches(0.5),
    Inches(6.7),
    Inches(12.3),
    Inches(0.4),
...
add_title(
    s,
    "Retrieval evaluation — and a surprise",
...
add_title(
    s,
    "Lesson 1: never use UNLOGGED tables for data you can't reconstruct",
...
for i, (when, what) in enumerate(timeline):
    y = Inches(2.1 + i * 0.72)
    add_text(s, when, Inches(0.8), y, Inches(2.2), Inches(0.5), size=14, bold=True, color=RED)
    add_text(s, what, Inches(3.2), y, Inches(9.5), Inches(0.6), size=14, color=INK)
...
add_title(
    s,
    "Lesson 2: the gap between naive and tuned pipelines is 10–20×",
...
add_title(
    s,
    "Lesson 3: LLM cost estimates are 3–5× too low",
...
def cost_row(y, label, bar_width, value, color):
    add_text(s, label, Inches(0.8), y, Inches(3.2), Inches(0.5), size=14, bold=True, color=INK)
    bar = s.shapes.add_shape(
        MSO_SHAPE.RECTANGLE, Inches(4.0), y + Inches(0.1), bar_width, Inches(0.45)
    )
    ...
    add_text(
        s,
        value,
        Inches(4.0) + bar_width + Inches(0.1),
        y + Inches(0.1),
        Inches(3.0),
        Inches(0.5),
        size=14,
...
add_text(
    s,
    "Entity extraction on 10K papers (Claude Haiku)",
    Inches(0.8),
    Inches(2.0),
    Inches(12),
    Inches(0.4),
...
add_text(
    s,
    "Why: a 500-token prompt extracting 50 tokens of structured output has 10× overhead. Caching recovers some; fine-tuning a smaller model may be necessary at full scale.",
    Inches(0.8),
    Inches(6.5),
    Inches(11.7),
    Inches(0.8),
...
s = add_blank(prs)
set_bg(s)
add_title(s, "What's next", sub="Research copilot gets deeper as the knowledge layer gets richer")
...
next_items = [
    ...
    (
        "Multi-corpus",
        "Same MCP surface over arXiv, bioRxiv, APS · " "agent writes corpus-agnostic code",
    ),
]
for i, (h, b) in enumerate(next_items):
    y = Inches(2.0 + i * 0.9)
    add_text(s, h, Inches(0.6), y, Inches(4.3), Inches(0.5), size=17, bold=True, color=ACCENT_DARK)
    add_text(s, b, Inches(5.0), y + Inches(0.05), Inches(7.8), Inches(0.9), size=13, color=INK)
...
checks = [
    ...
    ("Session memory", "Working set lives on the server. Provenance tags. Not in the prompt."),
    ("Tool count discipline", "< 15 tools. Compact stubs. Stable schemas. Cursor pagination."),
    ("Own your eval", "50 queries minimum. Name your methodology bias. Validate with experts."),
    (
...
for i, (h, b) in enumerate(checks):
    y = Inches(1.8 + i * 0.78)
    # check mark
    check = s.shapes.add_shape(
        MSO_SHAPE.OVAL, Inches(0.5), y + Inches(0.08), Inches(0.38), Inches(0.38)
    )
    ...
    add_text(s, h, Inches(1.05), y, Inches(4.0), Inches(0.5), size=16, bold=True, color=INK)
    add_text(
        s, b, Inches(5.3), y + Inches(0.05), Inches(7.7), Inches(0.7), size=13, color=INK_MUTED
    )
...
add_rule(s, 0, 0, Inches(0.18), height=SH, color=ACCENT)
add_text(
    s, "Thank you", Inches(0.9), Inches(1.2), Inches(12), Inches(1.4), size=64, bold=True, color=INK
)
add_text(
    s, "Questions?", Inches(0.9), Inches(2.7), Inches(12), Inches(0.8), size=28, color=ACCENT_DARK
)
