...
as they actually serve: it computes four lanes per query and fuses several
arms, holding the dense lane FIXED so the delta isolates the lexical swap.

Lanes (each top-50, ranked bibcodes):
  lex_pg  — Postgres ``scix_english`` tsvector      (search.lexical_search)
  lex_qd  — Qdrant BM25 sparse (pilot collection)   (scix_sparse_pilot_v1)
...
Arms (RRF k=60 over the lanes shown):
  dense_only         : [dense]
  pg_lex+dense       : [lex_pg, dense]          "lexical + qdrant hybrid"
  bm25+dense         : [lex_qd, dense]          "qdrant hybrid"
...
def lanes_for(conn, client, model, query: str, universe: set[str] | None) -> dict[str, list[str]]:
    ...
    return {
        "lex_pg": keep(_bibs(lexical_search(conn, query, filters=SearchFilters(), limit=fetch))),
        "lex_qd": keep([h.payload["bibcode"] for h in qd]),
        "body": keep(_bibs(lexical_search_body(conn, query, filters=SearchFilters(), limit=fetch))),
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument(
        ...
        "32M reach). Writes *_fair.{json,md}.",
    )
    ...
    log.info("restrict_universe=%s", args.restrict_universe)
    ...
    try:
        for q in queries:
            ...
            if not gold:
                continue
            lanes = lanes_for(conn, client, model, q["query"], universe)
            arms = {}
