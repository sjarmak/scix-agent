...
def measure(client: QdrantClient) -> dict:
    ...
    for i, p in enumerate(sample):
        nq = client.query_points(COLL, query=p.vector, limit=10,
            search_params=qm.SearchParams(quantization=qm.QuantizationSearchParams(ignore=True))).points
        if not any(x.id == p.id for x in nq):
            self_miss += 1
        if i < 200:  # exact GT is the slow part; 200 is plenty
            exact = client.query_points(COLL, query=p.vector, limit=10,
                search_params=qm.SearchParams(exact=True)).points
    ...
    return dict(self_miss=self_miss, n_self=len(sample),
                r10_mean=st.mean(r10), r10_min=min(r10))
...
def main() -> None:
    ...
    for name, r in results.items():
        print(f"{name}: build={r['build_s']:.0f}s  self-miss={r['self_miss']}/{r['n_self']}  "
