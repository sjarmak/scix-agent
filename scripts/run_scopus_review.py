#!/usr/bin/env python3
"""Run the bounded Scopus topic-by-venue review plan.

The private checkpoint contains DOI and Scopus EID identifiers only.  The
public summary contains aggregate counts and query provenance, with no record
identities or Scopus-returned descriptive content.
"""
...
from scix.review_sources.ieee_xplore import write_private_manifest
...
def _checkpoint_payload(
    *,
    run_date: str,
    plan_path: Path,
    results: list[ScopusSearchResult],
    status: str,
    ...
    return {
        ...
        "retention_boundary": (
            "Local DOI and Scopus EID identifiers only. No Scopus-returned title, "
            "abstract, author, or full-text content is retained."
        ),
...
def _restore_checkpoint(
    *,
    path: Path,
    plan_path: Path,
    plan_specs: tuple[ScopusSearchSpec, ...],
    run_date: str,
    client: ScopusClient,
    ...
    try:
        ...
        raise ScopusProtocolError("Could not read the Scopus review checkpoint") from exc
    if not isinstance(raw, dict):
        raise ScopusProtocolError("Scopus review checkpoint must be an object")
    expected_hash = hashlib.sha256(plan_path.read_bytes()).hexdigest()
    if raw.get("plan_sha256") != expected_hash:
        raise ScopusProtocolError("Scopus checkpoint belongs to a different search plan")
    if raw.get("source_lane") != "index_native_scopus":
        raise ScopusProtocolError("Scopus checkpoint has the wrong source lane")
    raw_results = raw.get("results")
    if not isinstance(raw_results, list):
        raise ScopusProtocolError("Scopus checkpoint results must be a list")
    if len(raw_results) > len(plan_specs):
        raise ScopusProtocolError("Scopus checkpoint contains too many results")
    restored = [
        ScopusSearchResult.from_private_dict(item, expected_spec=plan_specs[index])
    ...
    if raw.get("run_date") == run_date:
        ...
        if isinstance(prior_calls, bool) or not isinstance(prior_calls, int):
            raise ScopusProtocolError("Scopus checkpoint has an invalid call count")
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--call-limit", type=int, default=180)
