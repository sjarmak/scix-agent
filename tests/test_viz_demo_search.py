...
def test_demo_search_dispatches_through_mcp_call_tool(
    ...
    # Default mode is hybrid. The earlier SCIX_USE_HALFVEC gate (which forced
    # keyword while halfvec migrations 053/054 were pending) was removed once
    # empirical latency showed the float32 HNSW dense lane was fast enough, so
    # the demo always runs hybrid now. See demo_search() in scix/viz/api.py.
